...
class WeatherImageEncoder(nn.Module):
    """
    Weather classification encoder for image input.
    Args:
        c_dim (int): output dimension of the latent embedding for weather classification
    """
    ...
    def __init__(self, c_dim, normalize=True):
        ...
        # Modify the final fully connected layer to output the desired weather class embedding
        self.features.fc = nn.Linear(self.features.fc.in_features, c_dim)
...
class GPT(nn.Module):
    ...
    def __init__(self, n_embd, n_head, block_exp, n_layer,
        ...
        # weather embedding
        # 09.04
        self.weather_emb = nn.Linear(1, n_embd)
        ...
        self.weather_mode = config.weather_mode # config에서 임베딩을 넣을지 말지를 결정할 수 있음 
    ...
    def forward(self, image_tensor, lidar_tensor, velocity, weather_class):
        """
        Args:
        ...
        # project weather class to n_embed
        # print("weather_class: {}".format(weather_class.unsqueeze(1)))
        weather_class_tensors = weather_class.unsqueeze(1)
        weather_class_embeddings = self.weather_emb(weather_class.unsqueeze(1))
        ...
        if self.weather_mode:
            x = self.drop(
                self.pos_emb + token_embeddings + velocity_embeddings.unsqueeze(1) + weather_class_embeddings.unsqueeze(
                    1))  # (B, an * T, C)
        else:
            x = self.drop(
                self.pos_emb + token_embeddings + velocity_embeddings.unsqueeze(1))  # (B, an * T, C)
        '''
...
class Encoder(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Weather classification layer (09.08) if reuse fused_features?? 
        # self.weather_fc = nn.Linear(512, self.config.weather_classes)
    ...
    def forward(self, image_list, lidar_list, velocity, weather_class):
        '''
...
class TransFuser(nn.Module):
    ...
    def __init__(self, config, device):
        ...
        ## 09.08 added distince classification encoder 
        # Weather classification encoder (separate from main image encoder)
        self.weather_image_encoder = WeatherImageEncoder(config.weather_classes).to(self.device)
        ...
        ## 09.08 for classification weight
        # Weather loss weight for combining losses
        self.weather_loss_weight = config.weather_loss_weight

    def forward(self, image_list, lidar_list, target_point, velocity, weather_class):
        '''
        ...
        Args:
            ...
            velocity (tensor): input velocity from speedometer
        Modified(09.04) - weather class
            weather class (data['weather'] for embedding)
        '''
        # encoding info
        fused_features = self.encoder(image_list, lidar_list, velocity, weather_class)
        ...
        ## 09.08
        # Weather classification pass (only for RGB images)
        weather_image_tensor = torch.stack(image_list, dim=0).to(self.device)  # Assuming batch of images
        weather_logits = self.weather_image_encoder(weather_image_tensor)

        # Compute weather classification loss
        weather_loss = F.cross_entropy(weather_logits, weather_class)

        return pred_wp, weather_loss
    
    ## added 09.08
    def compute_loss(self, pred_wp, target_wp, weather_loss):
        '''
        Compute total loss from waypoint prediction and weather classification
        Args:
            pred_wp: predicted waypoints
            target_wp: ground truth waypoints
            weather_loss: classification loss from weather classification
        '''
        # Waypoint prediction loss (e.g., MSE loss)

        # loss = F.l1_loss(pred_wp, gt_waypoints, reduction='none').mean()
        # l1_loss modifiying

        wp_loss = F.l1_loss(pred_wp, target_wp, reduction='none').mean()

        # Combine losses with weather loss weight
        total_loss = wp_loss + self.weather_loss_weight * weather_loss
    ...
    def control_pid(self, waypoints, velocity):
        ...
        # print("desired_speed: {}, speed: {}".format(desired_speed, speed))
