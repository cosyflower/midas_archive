...
parser.add_argument('--id', type=str, default='not_weather_embedded', help='Unique experiment identifier.')
...
# setting directory and id, device, epochs, lr, val_freq, batch_size, logdir
...
# SummaryWriter에 기록하겠습니다. 이 때의 레이아웃으로 custom_scalars를 적용하겠습니다 
...
class Engine(object):
	...
	def __init__(self,  cur_epoch=0, cur_iter=0):
		...
		self.train_loss = []
		self.val_loss = []
		self.bestval = 1e10 # store lowest validation loss 
	...
	def train(self):
		...
		# Train loop
		# modified - only fronts and lidars (09.04)
		for data in tqdm(dataloader_train):
			...
			# driving labels from calra measurements (.json data)
			command = data['command'].to(args.device)
			# gt_weather (weather_id 정보를 가지고 오는 방법은 어떤지???)
			# gt_weather = data['weather'].to(args.device, dtype=torch.float32)
			gt_weather = gt_weather = data['weather_id'].to(args.device, dtype=torch.float32)
	...
	def validate(self):
		...
		with torch.no_grad():	
			...
			writer.add_scalar('loss/validation', wp_loss, self.cur_epoch)
