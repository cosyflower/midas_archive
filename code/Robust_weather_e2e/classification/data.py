...
class CARLA_Data(Dataset):
    def __init__(self, root, config, mode):
        ...
        for sub_root in tqdm(root, file=sys.stdout):
            ...
            if not os.path.exists(preload_file):
                ...
                for route in routes:
                    ...
                    if self.train_mode == True:
                        num_seq = (len(os.listdir(route_dir + "/rgb_front/")) // 3 - self.pred_len - 2) // self.seq_len
                    else:
                    ...
                    for seq_index in range(num_seq):
                        ...
                        for i in range(self.seq_len):
                            # images
                            filename = f"{str(seq + i).zfill(4)}.png"
                            ...
                            # point cloud
                            lidars.append(route_dir + f"/lidar/{str(seq + i).zfill(4)}.npy")

                            # position
                            with open(route_dir + f"/measurements/{str(seq + i).zfill(4)}.json",
                                      "r") as read_file:
                        ...
                        # should be modified (09.04) 
                        if data['weather']['fog_density'] < 80:
                            preload_weather.append(1)
                        else:
                            preload_weather.append(0)

                        # added (09.04)
                        # 날씨 늘릴꺼면 이 부분 추가로 수정해야 한다 
                        if data['weather_id'] == 'Night':
                            preload_weather_id.append(0)
                        else: # Sunny
                            preload_weather_id.append(1)


                        # read files sequentially (future frames)
                        for i in range(self.seq_len, self.seq_len + self.pred_len):
                            # point cloud
                            lidars.append(route_dir + f"/lidar/{str(seq + i*3).zfill(4)}.npy")

                            # position
                            with open(route_dir + f"/measurements/{str(seq + i*3).zfill(4)}.json",
                                      "r") as read_file:
                                data = json.load(read_file)
                            xs.append(data['x'])
                            ys.append(data['y'])
                ...
                preload_dict['weather'] = preload_weather
                # added (09.04)
                preload_dict['weather_id'] = preload_weather_id
            ...
            self.weather_condition += preload_dict.item()['weather']
            # Modified(09.04)
            self.weather_id += preload_dict.item()['weather_id']
    ...
    def __getitem__(self, index):
        ...
        data['weather'] = self.weather_condition[index]
        # added (09.04)
        data['weather_id'] = self.weather_id[index]
...
def lidar_to_histogram_features(lidar, crop=256):
    ...
    # print("below_features: {}".format(np.mean(below_features)))
    # print("above_features: {}".format(np.mean(above_features)))
    # plt.subplot(1, 2, 1)
    # plt.imshow(below_features)
    # plt.subplot(1, 2, 2)
    # plt.imshow(above_features)
    # plt.show()
    return features
